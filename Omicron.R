...
plot.odds.ratio<-function(glm.model,log.coef=FALSE,as.is=FALSE){
  ...
  if(isTRUE(log.coef)){
    ...
    mtext(bquote("Odds ratio ("~"e"^beta~")"),side=1,line=2.5)
  }
...
Om.data.corr<-Om.data %>% mutate(result=ifelse(ORF1ab_Ct>30,"negative",result)) %>% rowwise() %>%
  ...
  mutate(dose1=ifelse(Doses==1,1,0),
         age=ifelse(age==">=90","90",age),sex=ifelse(sex=="","Unknown",sex),
  ...
  ##dplyr::filter(vax_1_mfr!="unknown" | vax_2_mfr!="unknown") %>%
  ##dplyr::filter(vax_1_mfr!="unknown" | vax_2_mfr!="unknown") %>%
  ...
  mutate(vax_status=fct_recode(vax_status,"unvax"="0","vax_1d"="1","vax_2d_6m"="2","vax_2d_6m+"="3","3d"="4","Unknown"="Unknown")) %>%
  ...
  mutate(vax_status_=ifelse(Doses==0,"unvax",
                            ifelse(Doses==1 & as.numeric(days_from_1st_dose)/30<5 & vax_1_mfr=="1","jj_1d_6m",
                            ifelse(Doses==1 & as.numeric(days_from_1st_dose)/30>=5 & vax_1_mfr=="1","jj_1d_6m+",
                            ifelse(Doses==1 & as.numeric(days_from_1st_dose)/30<5 & vax_1_mfr=="2","pf_1d_6m",
                            ifelse(Doses==1 & as.numeric(days_from_1st_dose)/30>=5 & vax_1_mfr=="2","pf_1d_6m+",
                            ifelse(Doses==1 & as.numeric(days_from_1st_dose)/30<5 & vax_1_mfr=="3","mod_1d_6m",
                            ifelse(Doses==1 & as.numeric(days_from_1st_dose)/30>=5 & vax_1_mfr=="3","mod_1d_6m+",
                            ifelse(Doses==2 & as.numeric(days_from_2nd_dose)/30<5 & vax_1_mfr=="1","jj_2d_6m",
                            ifelse(Doses==2 & as.numeric(days_from_2nd_dose)/30>=5 & vax_1_mfr=="1","jj_2d_6m+",
                            ifelse(Doses==2 & as.numeric(days_from_2nd_dose)/30<5 & vax_1_mfr=="2" & vax_2_mfr=="2","pf_2d_6m",
                            ifelse(Doses==2 & as.numeric(days_from_2nd_dose)/30>=5 & vax_1_mfr=="2" & vax_2_mfr=="2","pf_2d_6m+",
                            ifelse(Doses==2 & as.numeric(days_from_2nd_dose)/30<5 & vax_1_mfr=="3" & vax_2_mfr=="3","mod_2d_6m",
                            ifelse(Doses==2 & as.numeric(days_from_2nd_dose)/30>=5 & vax_1_mfr=="3" & vax_2_mfr=="3","mod_2d_6m+",
                            ifelse(Doses>=3,"3d","unknown") )))))))))))))) %>% ungroup() %>%
  dplyr::filter(sex!="Unknown") %>% 
  dplyr::filter(vax_status!="Unknown") %>% 
...
Om.data.corr.ct<-Om.data %>% inner_join(sid.tmp,by="hash_subject_id") %>% mutate(result=ifelse(ORF1ab_Ct>30,"negative",result)) %>%
  dplyr::rename(S_Ct=`S-gene_Ct`,test_result=result) %>% 
  mutate(dose1=ifelse(Doses==1,1,0),sex=ifelse(sex=="","Unknown",sex),
         age=ifelse(age==">=90","90",age),
         days_from_2nd_dose=as.Date(measurement_date,"%m/%d/%y")-as.Date(vax_2_date,"%m/%d/%y"),
         days_from_1st_dose=as.Date(measurement_date,"%m/%d/%y")-as.Date(vax_1_date,"%m/%d/%y"),
         days_from_3rd_dose=as.Date(measurement_date,"%m/%d/%y")-as.Date(booster_1_date,"%m/%d/%y")) %>%
  dplyr::filter(vax_1_mfr!="unknown" | vax_2_mfr!="unknown") %>%
  dplyr::filter(test_result!="negative") %>%
  dplyr::filter(test_result!="negative") %>%
  mutate(vax_1_mfr=ifelse(vax_1_mfr=="J&J","1",
  ...
  ##dplyr::filter(!is.na(as.double(ORF1ab_Ct))) %>% 
  mutate(ORF1ab_Ct=as.double(ORF1ab_Ct),id2=as.factor(id2)) %>%
  ####dplyr::select(ORF1ab_Ct,test_result,vax_status,id2,vax_status_,age,sex) %>% drop_na() %>% dplyr::filter(sex!="Unknown") %>%
  mutate(sex=as.factor(sex)) %>%
  mutate(vax_status_=fct_relevel(vax_status_,"unvax",after=0),
         sex=fct_relevel(sex,"Female",after=0),
         ORF1ab_Ct=as.double(ORF1ab_Ct),age=as.double(age)) 
...

{
  ...
  {
    par(mai=mai.mat)
    glm.out.ct.gee<-gee(ORF1ab_Ct~test_result+vax_status+age+sex,data=Om.data.corr.ct,id=id2,corstr="unstructured",family=gaussian)
    glm.out.ct<-as.data.frame(coef(summary(glm.out.ct.gee)))
    glm.out.ct[,"Pr(>|z|)"]<-unname(2*pnorm(abs(glm.out.ct[,5]),lower.tail=FALSE))
    
    out.gee<-glm.out.ct %>% rownames_to_column("term") %>% 
      mutate(conf.low=Estimate-1.96*`Robust S.E.`,
             conf.high=Estimate+1.96*`Robust S.E.`,
             std.error=`Robust S.E.`,
             p.value=`Pr(>|z|)`,statistic=`Robust z`) %>%
    ...
    plot.odds.ratio(out.gee,as.is=TRUE)
    title(bquote("Nasal swab PCR C"[t]~"-value"),font.main=1,cex.main=1.0,line=1)
    
    out.gee %>% dplyr::filter(term!="(Intercept)") %>% 
  ...
  {
    par(mai=mai.mat)
    glm.out.ct.gee<-gee(ORF1ab_Ct~test_result+vax_status_+age+sex,data=Om.data.corr.ct,id=id2,corstr="unstructured",family=gaussian)
    glm.out.ct<-as.data.frame(coef(summary(glm.out.ct.gee)))
    glm.out.ct[,"Pr(>|z|)"]<-unname(2*pnorm(abs(glm.out.ct[,5]),lower.tail=FALSE))
...
plot.odds.ratio(glm.out1,log.coef=TRUE)

plot.odds.ratio(glm.out2,log.coef=FALSE)
...

{
  ...
  {
    Om.data.tmp<-Om.data %>% mutate(test_result=ifelse(ORF1ab_Ct>30,"negative",result)) %>%
      mutate(dose1=ifelse(Doses=="Unknown",1,0),
             days_from_2nd_dose=as.Date(measurement_date,"%m/%d/%y")-as.Date(vax_2_date,"%m/%d/%y"),
             days_from_1st_dose=as.Date(measurement_date,"%m/%d/%y")-as.Date(vax_1_date,"%m/%d/%y"),
             days_from_3rd_dose=as.Date(measurement_date,"%m/%d/%y")-as.Date(booster_1_date,"%m/%d/%y")) %>%
      mutate(Doses=ifelse(Doses==2 & days_from_2nd_dose/30<5,"2d6m",
                          ifelse(Doses==2 & days_from_2nd_dose/30>=5,"2d6m+",Doses))) %>%
      mutate(Doses=ifelse(is.na(Doses),"Unknown",Doses ))
    
    {
      case.counts<-table(Om.data.tmp$test_result,Om.data.tmp$Doses)
      case.counts<-cbind(case.counts,"ALL"=rowSums(case.counts))[,c("ALL","0","1","2d6m+","2d6m","3","Unknown")]
      case.totals<-rbind(colSums(case.counts),colSums(case.counts),colSums(case.counts),colSums(case.counts))
      case.totals<-case.totals[,c("ALL","0","1","2d6m+","2d6m","3")]
      case.totals
      case.counts<-rbind(case.counts,"positive"=colSums(case.counts[-2,]))[,c("ALL","0","1","2d6m+","2d6m","3")]
      case.counts
      ...
      colnames(tmp.df)<-c("ALL","0","1","2d6m+","2d6m","3")
      prop.vals<-tmp.df[!rownames(tmp.df) %in% c("negative"),] 
      prop.vals %>% dplyr::select("ALL","0","1","2d6m+","2d6m","3") %>%
        write.table(file="test.pos.rate.tsv",sep="\t",row.names=FALSE)
    }
    
    prop.test(c(869,1023),c(19522,19522))$p.value
    ...
    wdr<-barplot(var.prop,las=1,ylim=c(min(ylab.vals),max(ylab.vals)),
                 col=viridis::inferno(5)[c(4,3)],beside = TRUE,yaxt="n",
    ...
    {
      Om.data.tmp<-Om.data %>% inner_join(sid.tmp,by="hash_subject_id") %>% mutate(result=ifelse(ORF1ab_Ct>30,"negative",result)) %>%
        dplyr::rename(S_Ct=`S-gene_Ct`,test_result=result) %>% 
        mutate(dose1=ifelse(Doses==1,1,0),sex=ifelse(sex=="","Unknown",sex),
               age=ifelse(age==">=90","90",age),
               days_from_2nd_dose=as.Date(measurement_date,"%m/%d/%y")-as.Date(vax_2_date,"%m/%d/%y"),
        ...
        mutate(vax_1_mfr=ifelse(vax_1_mfr=="J&J","1",
                                ifelse(vax_1_mfr=="Pfizer","2",
                                       ifelse(vax_1_mfr=="Moderna","3","unknown")))) %>%
        mutate(vax_2_mfr=ifelse(vax_2_mfr=="J&J","1",
                                ifelse(vax_2_mfr=="Pfizer","2",
                                       ifelse(vax_2_mfr=="Moderna","3","")))) %>%
        mutate(booster_1_mfr=ifelse(booster_1_mfr=="J&J","1",
        ...
        ####dplyr::select(ORF1ab_Ct,test_result,vax_status,id2,vax_status_,age,sex) %>% drop_na() %>% dplyr::filter(sex!="Unknown") %>%
        mutate(sex=as.factor(sex)) %>%
        mutate(sex=fct_relevel(sex,"Female",after=0),
               age=as.double(age)) %>%
        mutate(Doses=ifelse(is.na(Doses),"Unknown",Doses )) %>% ##vax_status_=fct_relevel(vax_status_,"unvax",after=0),
        mutate(vax_status_=ifelse(is.na(vax_status_),"Unknown",vax_status_ ))
      
      {
        case.counts<-table(Om.data.tmp$test_result,Om.data.tmp$vax_status_)
        case.counts<-cbind(case.counts,"ALL"=rowSums(case.counts))[,c("ALL","unvax","jj_1d_6m+","jj_1d_6m","mod_1d_6m","mod_2d_6m+","mod_2d_6m","pf_1d_6m+","pf_1d_6m","pf_2d_6m+","pf_2d_6m","3d","unknown")]
        case.totals<-rbind(colSums(case.counts),colSums(case.counts),colSums(case.counts),colSums(case.counts))
        case.totals<-case.totals[,c("ALL","unvax","jj_1d_6m","jj_1d_6m+","mod_1d_6m","pf_1d_6m+","pf_1d_6m","mod_2d_6m","mod_2d_6m+","pf_2d_6m+","pf_2d_6m","3d")]
        case.totals
        case.counts<-rbind(case.counts,"positive"=colSums(case.counts[-2,]))[,c("ALL","unvax","jj_1d_6m","jj_1d_6m+","mod_1d_6m","pf_1d_6m+","pf_1d_6m","mod_2d_6m","mod_2d_6m+","pf_2d_6m+","pf_2d_6m","3d")]
        case.counts
  ...
  {
    par(mai=mai.vals)
    glm.out.var.gee<-gee(test_result~vax_status+age+sex,data=Om.data.corr.ct,id=id2,corstr="unstructured",family=binomial)
    glm.out.var<-as.data.frame(coef(summary(glm.out.var.gee)))
    glm.out.var[,"Pr(>|z|)"]<-unname(2*pnorm(abs(glm.out.var[,5]),lower.tail=FALSE))
    
    out.gee<-glm.out.var %>% rownames_to_column("term") %>% 
      mutate(conf.low=Estimate-1.96*`Robust S.E.`,
             conf.high=Estimate+1.96*`Robust S.E.`,
             std.error=`Robust S.E.`,
             p.value=`Pr(>|z|)`,statistic=`Robust z`) %>%
  ...
  {
    par(mai=mai.vals1)
    glm.out.var.gee<-gee(test_result~vax_status_+age+sex,data=Om.data.corr.ct,id=id2,corstr="unstructured",family=binomial)
    glm.out.var<-as.data.frame(coef(summary(glm.out.var.gee)))
    glm.out.var[,"Pr(>|z|)"]<-unname(2*pnorm(abs(glm.out.var[,5]),lower.tail=FALSE))
...

{
  ...
  mai.vals<-c(0.761,0.751,0.161,0.150)
  par(mai=mai.vals )
